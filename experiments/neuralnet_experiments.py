...
class ExperimentSet(gsim.AbstractExperimentSet):
        ...
        class ExampleDataset(Dataset):
            def __init__(self, num_examples):
                ...
                self.m_targets = torch.sum(
                    self.m_feat, dim=1, keepdim=True
                ) + 0.5 * torch.randn(num_examples, 1)
    ...
    def experiment_1002(l_args):
        class MyDataset(Dataset):
            def __init__(self, num_examples):
                ...
                self.m_targets = MyDataset.target_fun(self.m_feat) + 100 * torch.randn(
                    num_examples, 1
                )
        ...
        def plot_data_distribution(dataset: MyDataset) -> GFigure:
            G = GFigure(
                xlabel="Feature value", ylabel="Histogram", num_subplot_columns=1
            )
        ...
        for d_net in l_nets:
            ...
            optimizer = torch.optim.AdamW(
                net.parameters(), lr=d_net["lr"], weight_decay=d_net["weight_decay"]
            )
    ...
    def experiment_1003(l_args):
        ...
        def plot_data():
            preds = [
                float(p[0])
                for p in net.predict(test_dataset, dataset_includes_targets=True)
            ]
    ...
    def experiment_1004(l_args):
        ...
        class FeatureVectorDataset(Dataset):
            ...
            def __init__(self, num_examples):
                ...
                self.m_feat = torch.cat([feat_0, feat_1, feat_2, feat_3], dim=1)
        ...
        normalizer = MultiFeatNormalizer(
            input_normalizers=[
                ...
                StdFeatNormalizer(),  # Feature 1: standardize (remove mean, divide by std)
                IntervalFeatNormalizer(interval=(0, 1)),  # Feature 2: scale to [0,1]
